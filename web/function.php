...
//    }
	...
	function getBirthdayAndGenderForNIC($year, $days)
	{
	    if ($year < 1900) {
	        return false;
	    }
	    ...
	    if ($days > 0 && $days <= $maxDays) {
	        $gender = "Male";
	    } elseif ($days > 500 && $days <= 500 + $maxDays) {
	        $gender = "Female";
	        $days -= 500;
	    } else {
	        return false;
	    ...
	    $birthdate = date_create_from_format('Y-z', $year . '-' . $days);
	    if (!$birthdate) {
	        return false;
	    }
	    $birthdate->modify('-2 days');
	    return array('dob' => $birthdate->format('Y-m-d'), 'gender' => $gender);
	}

	function validateNIC($nic)
	{
	    $oldnic_pattern = "/^[0-9]{9}[VXvx]$/";
	    $newnic_pattern = "/^[0-9]{12}$/";

	    if (preg_match($oldnic_pattern, $nic)) {
	        $birthyear = '19' . substr($nic, 0, 2);
	        $days = substr($nic, 2, 3);
	        return getBirthdayAndGenderForNIC($birthyear, $days);
	    }

	    if (preg_match($newnic_pattern, $nic)) {
	        $birthyear = substr($nic, 0, 4);
	        $days = substr($nic, 4, 3);
	        return getBirthdayAndGenderForNIC($birthyear, $days);
	    }
    ...
    function validateTextFieldswithDigits($value){
        if(!preg_match('/^(?=.*[a-zA-Z])(?=.*\d)[a-zA-Z\d, ]+$/', $value) && !preg_match('/^[A-Z][a-z]*(\s+[A-Z][a-z]*)*$/', $value)){
            return TRUE;
        }
    }
