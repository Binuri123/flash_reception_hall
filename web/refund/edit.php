...
<main id="main" class="main">
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="<?= WEB_PATH ?>refund/refund.php">Refund</a></li>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        extract($_GET);
        $db = dbConn();
        $sql = "SELECT r.reservation_no,r.discounted_price,r.event_date,rr.refund_method_id FROM refund_request rr "
                . "LEFT JOIN reservation r ON r.reservation_no=rr.reservation_no WHERE refund_request_id = '$request_id'";
        $result = $db->query($sql);
        $row = $result->fetch_assoc();
        $reservation_no = $row['reservation_no'];
        $reservation_price = $row['discounted_price'];
        $refund_method = $row['refund_method_id'];

        if ($refund_method == '1') {
            $sql_cash_refund = "SELECT * FROM cash_refund WHERE refund_request_id='$request_id'";
            $result_cash_refund = $db->query($sql_cash_refund);
            $row_cash_refund = $result_cash_refund->fetch_assoc();
            $cash_collector = $row_cash_refund['cash_collector'];
            if ($cash_collector == 'other') {
                $collector_name = $row_cash_refund['cash_collector_name'];
                $collector_nic = $row_cash_refund['cash_collector_nic'];
            }
        } elseif ($refund_method == '2') {
            $sql_bank_refund = "SELECT * FROM bank_refund WHERE refund_request_id='$request_id'";
            $result_bank_refund = $db->query($sql_bank_refund);
            $row_bank_refund = $result_bank_refund->fetch_assoc();
            $bank_name = $row_bank_refund['bank_id'];
            $bank_branch = $row_bank_refund['branch_name'];
            $account_holder = $row_bank_refund['account_holder_name'];
            $account_number = $row_bank_refund['account_number'];
        }
    }
    extract($_POST);
    //var_dump($_POST);
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && @$action == 'save') {
        ...
        if (empty($message)) {
            ...
            $sql = "UPDATE refund_request SET reservation_no='$reservation_no',customer_no='$customer_no',paid_amount='$paid_amount',refundable_amount='$refundable_amount',"
                    . "refund_method_id='$refund_method',refund_status_id='2',update_date='$cDate',update_user='$user_id' WHERE refund_request_id = '$request_id'";
            $db->query($sql);

            $sql = "DELETE FROM bank_refund WHERE refund_request_id = '$request_id'";
            $db->query($sql);

            $sql = "DELETE FROM cash_refund WHERE refund_request_id = '$request_id'";
            $db->query($sql);

            if ($refund_method == '2') {
                echo $sql = "INSERT INTO bank_refund(refund_request_id,bank_id,branch_name,account_holder_name,account_number) "
                        . "VALUES('$request_id','$bank_name','$bank_branch','$account_holder','$account_number')";
            }

            if ($refund_method == '1') {
                $sql = "INSERT INTO cash_refund(refund_request_id,cash_collector,cash_collector_name,cash_collector_nic) "
                        . "VALUES('$request_id','$cash_collector','$collector_name','$collector_nic')";
            }

            $db->query($sql);

            header('location:refund_request_update_success.php?request_id=' .$request_id);
        }
    }
    ?>
                        ...
                        <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>" enctype="multipart/form-data">
                            <div class="row">
                                ...
                                <div class="col-md-7 mb-3 mt-3">
                                    <?php
                                    $db = dbConn();
                                    $sql_reservation_no = "SELECT reservation_no FROM refund_request WHERE refund_request_id='$request_id'";
                                    $result_reservation_no = $db->query($sql_reservation_no);
                                    $row_reservation_no = $result_reservation_no->fetch_assoc();
                                    $reservation_no = $row_reservation_no['reservation_no'];
                                    ?>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7 mb-3">
                                    <?php
                                    $db = dbConn();
                                    $sql_reservation_date = "SELECT event_date FROM reservation WHERE reservation_no = (SELECT reservation_no FROM refund_request WHERE refund_request_id='$request_id')";
                                    $result_reservation_date = $db->query($sql_reservation_date);
                                    $row_reservation_date = $result_reservation_date->fetch_assoc();
                                    $reservation_date = $row_reservation_date['event_date'];
                                    ?>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7 mb-3">
                                    <?php
                                    $db = dbConn();
                                    $sql_reservation_price = "SELECT discounted_price FROM reservation WHERE reservation_no = (SELECT reservation_no FROM refund_request WHERE refund_request_id='$request_id')";
                                    $result_reservation_price = $db->query($sql_reservation_price);
                                    $row_reservation_price = $result_reservation_price->fetch_assoc();
                                    $reservation_price = $row_reservation_price['discounted_price'];
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7 mb-3">
                                    <?php
                                    $sql_paid_amount = "SELECT SUM(paid_amount) AS total_paid_amount FROM customer_payments WHERE reservation_no = '$reservation_no' AND (payment_status = '2' OR payment_status = '4')";
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7 mb-3">
                                    <?php
                                    $sql_add_time = "SELECT add_date,add_time FROM reservation WHERE reservation_no ='$reservation_no'";
                                    ...
                                    $sql_cancel_time = "SELECT canceled_date,cancel_time FROM canceled_reservations WHERE reservation_no ='$reservation_no'";
                                    ...
                                    $sql_made_payments = "SELECT MAX(payment_category_id) as last_payment "
                                            . "FROM customer_payments WHERE reservation_no='$reservation_no' "
                                    ...
                                    if ($timeDifferenceInHours < 4 && $last_payment >= '1') {
                                        $refundable_amount = $paid_amount;
                                        $notice = "You have Canceled Your Reservation Within 4 Hours. "
                                                . "You are Eligible to a Full Refunded According to the Reservation Policies.";
                                    } elseif ($timeDifferenceInHours > 4 && $last_payment > '1') {
                                        $sql = "SELECT amount FROM payment_category WHERE payment_category_id='1'";
                                        $result = $db->query($sql);
                                        $row = $result->fetch_assoc();
                                        $security_fee = $row['amount'];
                                        $refundable_amount = $paid_amount - $security_fee;
                                        $notice = "You have Canceled Your Reservation After 4 Hours. "
                                                . "The Security Fee Will Not be Refunded According to the Reservation Policies.";
                                    }
                                    ?>
                                    <div><?= number_format($refundable_amount, '2', '.', ',') ?></div>
                                    <p><strong><i><?= @$notice ?></i></strong></p>
                                    <input type="hidden" name="refundable_amount" value="<?= str_replace(',', '', @$refundable_amount) ?>" id="refundable_amount">
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-5 mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-12" style="text-align:right">
                                    ...
                                    <button type="cancel" class="btn btn-warning btn-sm" style="font-size:13px;width:150px;">Cancel</button>
