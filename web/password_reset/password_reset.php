...
//var_dump($_POST);
    if ($_SERVER['REQUEST_METHOD'] == "POST" && @$action == 'reset_password') {
        ...
        if (empty($message['error_password'])) {
            $db = dbConn();
            echo $sql = "SELECT password FROM users where user_id=(SELECT user_id FROM customer WHERE customer_id='$customer_id')";
            $result = $db->query($sql);
        ...
        if (empty($message)) {
            //$password = sha1($password);
            echo $sql = "UPDATE users SET password ='$password' WHERE user_id = (SELECT user_id FROM customer WHERE reg_no='$reg_no')";
            $db->query($sql);
                    ...
                    <div class="col-md-4 h-90 mt-3">
                        <div class="card bg-white text-dark h-90" style="border-radius:10px;">
                            <div class="card-body text-center">
                                    <div>
                                        <h2 class="fw-bold mb-2 mb-3"><i class="bi bi-lock-fill"></i> Reset Your Password</h2>
