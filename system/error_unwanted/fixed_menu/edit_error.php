...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if($_SERVER['REQUEST_METHOD']=='GET'){
        ...
        $sql = "SELECT mi.item_id,mi.item_name,mi.item_price,mc.category_id,mc.category_name FROM menu_category_item mci INNER JOIN menu_item mi ON mi.item_id=mci.item_id INNER JOIN menu_item_category mc ON mc.category_id=mci.category_id WHERE mci.menu_id=$menu_id";
        $result = $db->query($sql);
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST" && @$action == 'save') {
        ...
        if (empty($message)) {
            ...
            header('Location:edit_success.php?menu_id='.$menu_id. '&values=' . urlencode($updated_fields_string));
        }
    }
    ?>
    <div class="row">
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-10 mb-3" style="text-align:right">
                                    <button type="button" class="btn btn-sm btn-success" onclick="form.submit()" name="action" value="add_item">Add</button>
                                </div>
                                <div class="col-md-1"></div>
                            </div>
                            <div class="row">
                                        ...
                                        <tbody>
                                            <?php
                                                if($_SERVER['REQUEST_METHOD'] == 'POST' && @$action== 'remove_item'){
                                                   //@$vendor_price -= $_SESSION['selected_items'][@$_POST[item_id]]['item_price'];
                                                    unset($_SESSION['selected_items'][@$_POST['item_id']]);
                                            ...
                                            <?php
                                                    ...
                                                    }else if(!empty($_SESSION['selected_items'])){
                                                    @$vendor_price = 0;
                                                    foreach($_SESSION['selected_items'] as $unit){
                                                        @$vendor_price += $unit['item_price'];
                                            ?>
