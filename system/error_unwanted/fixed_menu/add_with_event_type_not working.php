...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group me-2">
                <a class="btn btn-sm btn-outline-success" href="<?= SYSTEM_PATH ?>menu_package/fixed_menu/fixed_menu.php"><span data-feather="calendar" class="align-text-bottom"></span> Search Menu</a>
            </div>
        </div>
    </div>
    <?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST" && @$action == 'save') {
        ...
        if (empty($event_type)) {
            $message['error_event_type'] = "The Event Types Should Be Selected...";
        }
        ...
        if (empty($message)) {
            ...
            $cDate = date('Y-m-d');
            echo $sql = "INSERT INTO menu_package(menu_package_name,menu_type_id,event_menu_type_id,total_price,profit_ratio,final_price,availability,add_user,add_date)VALUES('$menu_name','1','$event_menu_type','$total_price','$profit_ratio','$final_price','$availability','$userid','$cDate')";
            $db->query($sql);
                    ...
                    <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>" enctype="multipart/form-data">
                        <div class="row">
                            <div class="col-md-5">
                                ...
                                <div class="row">
                                    <div class="col-md-4 mb-3">
                                        <label for="event_type" class="form-label"><span class="text-danger">*</span> Menu Type</label>
                                    </div>
                                    <div class="col-md-8 mb-3">
                                        <?php
                                        $event_type = array();
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            <div class="form-check form-check">
                                                <input class="form-check-input" type="checkbox" id="<?= $row['event_id'] ?>" name="event_type[]" value="<?= $row['event_id'] ?>" <?php if (isset($event_type) && in_array($row['event_id'],$event_type)) { ?>checked <?php } ?>>
                                                <label class="form-check-label" for="<?= $row['event_id'] ?>" style="font-size:13px;"><?= $row['event_name'] ?></label>
                                            </div>
                                            <?php
                                        }
                                        ?>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-8 mb-3">
                                        ...
                                        if (!empty(@$total_price) && !empty(@$profit_ratio)) {
                                            $final_price = @$total_price + @$total_price * @$profit_ratio / 100;
