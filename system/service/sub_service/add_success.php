...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == "GET") {
        ...
        if (!empty($sample_id)) {
            $db = dbConn();
            $sql = "SELECT sc.category_name,s.service_name,ss.sample_name,ss.availability,ss.sample_image FROM service_samples ss "
                    . "LEFT JOIN service s ON s.service_id=ss.service_id "
                    . "LEFT JOIN service_category sc ON sc.category_id=ss.service_category_id WHERE service_sample_id='$sample_id'";
            //print_r($sql);
            $result = $db->query($sql);
